...
export default {
  ...
  methods: {
    ...
    goCardHuan () {
      var that = this;
      that.getMyInfo(that, that.goCardHuanCb)
  ...
  activated: function () { // 加载当前路由的时候执行 其余的都是 初始化项目的时候加载
    ...
    this.realData = [
      ...
      { name: '身份证', model: this.userData.identityNo, tag: 'certId', type: 'ident', placeholder: '请输入身份证', length: '18', error: false, errorMsg: '请输入正确的身份证号', reg: '' },
